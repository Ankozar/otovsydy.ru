...
// $source_link_first_part = 'https://tyumen.sledcom.ru';
// $sourceNameInDb = 'source_sledkom';
// $site = file_get_contents($source_url);
// $document = phpQuery::newDocument($site);
// echo $document;

 $news_main_block = $document->find('.b-list');
 $news_blocks = $news_main_block->find('.bl-item');
 ...
 foreach($news_blocks as $block){
    ...
    preg_match("/\w.*\w\D/u", $text_title_wrong[$i], $text_title);
    echo $text_title[0] . '<br>';
    $i++;
 };
